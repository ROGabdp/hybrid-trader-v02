...
"""
================================================================================
V3 模型訓練腳本 - 輕量化微調版本 (Lightweight)
================================================================================
本腳本執行完整的 V3 訓練流程：
1. 清除舊的特徵快取（強制用新的 30 次 MC Dropout 採樣重算）
2. 執行預訓練 (Pre-training)
3. 執行微調 (Fine-tuning) - 輕量化步數

與 V4 的差異：
- Buy Agent Fine-tune: 200K 步 (輕量化)
- Sell Agent Fine-tune: 100K 步 (輕量化)
...
def run_finetuning():
    """Step C: 微調 (輕量化)"""
    print("\n" + "=" * 60)
    print("🎯 Step C: 微調 (Fine-tuning - 輕量化)")
    print("=" * 60)
    print(f"  Buy Agent:  {FINETUNE_BUY_STEPS:,} steps (輕量化)")
    print(f"  Sell Agent: {FINETUNE_SELL_STEPS:,} steps (輕量化)")
    print(f"  輸出路徑:   {V3_MODELS_PATH}")
    print("=" * 60)
